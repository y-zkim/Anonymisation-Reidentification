...
def main():
    ...
    for i in df['id']:
        ...
        if(actual_hour in (0, 2)):
            df['Date'][tmp] = df['Date'][tmp] + datetime.timedelta(seconds= np.random.randint(0, 29), hours=np.random.randint(1, 3))
        ...
        df['longitude'][tmp] += float(format(np.random.uniform(0.0001, 0.0006), ".6f"))
        df['lattitude'][tmp] += float(format(np.random.uniform(0.0001, 0.0006), ".6f"))
